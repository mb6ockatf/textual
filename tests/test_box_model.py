...
def test_content_box():
    ...
    assert box_model == BoxModel(
        Fraction(10), Fraction(8), Spacing(0, 0, 0, 0)
    )
    ...
    assert box_model == BoxModel(
        Fraction(14), Fraction(12), Spacing(0, 0, 0, 0)
    )
...
def test_width():
    ...
    class TestWidget(Widget):
        ...
        def get_content_height(
            self, container: Size, parent: Size, width: int
        ) -> int:
            return 10
    ...
    box_model = widget._get_box_model(Size(60, 20), Size(80, 24), one, one)
    assert box_model == BoxModel(
        Fraction(60), Fraction(20), Spacing(0, 0, 0, 0)
    )
    ...
    box_model = widget._get_box_model(Size(60, 20), Size(80, 24), one, one)
    assert box_model == BoxModel(
        Fraction(54), Fraction(16), Spacing(1, 2, 3, 4)
    )
    ...
    assert box_model == BoxModel(
        Fraction(10), Fraction(16), Spacing(1, 2, 3, 4)
    )
    ...
    box_model = widget._get_box_model(Size(60, 20), Size(80, 24), one, one)
    assert box_model == BoxModel(
        Fraction(80), Fraction(16), Spacing(1, 2, 3, 4)
    )
...
def test_height():
    ...
    box_model = widget._get_box_model(Size(60, 20), Size(80, 24), one, one)
    assert box_model == BoxModel(
        Fraction(54), Fraction(24), Spacing(1, 2, 3, 4)
    )
    ...
    print(box_model)
    assert box_model == BoxModel(
        Fraction(56), Fraction(10), Spacing(2, 2, 2, 2)
    )
...
def test_max():
    ...
    assert box_model == BoxModel(
        Fraction(40), Fraction(30), Spacing(0, 0, 0, 0)
    )
